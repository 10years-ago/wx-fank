...
<script>
	export default {
		...
		created() {
			...
			try{
				const value = uni.getStorageSync('baiduToken')
				if(value){
					apiKey = value.apiKey
				...
				uni.chooseImage({
					...
					success: function(e) {
						...
						wx.getFileSystemManager().readFile({
							...
							success:e=>{
								uni.request({
									url: 'https://aip.baidubce.com/rest/2.0/ocr/v1/idcard?access_token=' + _this.baiduToken,
									data:{
										image:e.data,
										id_card_side: 'front'
									},
									...
									success :e => {
										if(e.data.image_status == 'normal'){
											_this.$emit('baidu',e.data.words_result)
										}else if(e.data.image_status == 'blurred'){
											_this.$emit('alert-show','请上传清晰的身份证照片')
										}else{
											_this.$emit('alert-show','请在第一张图片处放置身份证正面(头像面)')
										}
									}
								})
							}
